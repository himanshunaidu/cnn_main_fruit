...
#Take in RGB images
convo1 = convolutional_layer(x_image, shape=[5, 5, 3, 96], stridec=[1, 4, 4, 1])
convo1_relu = tf.nn.relu(convo1)
#lrn1 = LRN(convo1, 2, 2e-05, 0.75, "norm1")
convo1_pool = max_pool_2by2(convo1_relu)

convo2 = convolutional_layer(convo1_pool, shape=[3, 3, 96, 192])
...
convo2_flat = tf.reshape(convo2_pool_drop, [-1, 7*7*192])
full_layer_one = tf.nn.relu(normal_full_layer(convo2_flat, 1000))
...
var_export_dir = '<PATH>/TempFM2/variables.ckpt'
saver = tf.train.Saver()

test_saver = open('<PATH>/TempFM2/test.txt', 'a+')
...
time_saver = open('<PATH>/TempFM2/time.txt', 'a+')
